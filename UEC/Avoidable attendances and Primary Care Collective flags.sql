...
-- 	 then '1' else '0' 
	...
	--Admitted/Non admitted -- Might not be in line with National Definition
...
--[Reporting_MESH_ECDS].[EC_Core_snapshot] as ED --monthly snapshot of reporting table
		--look up Provider
left join [UKHD_ODS].[Provider_Hierarchies_ICB]  as vOPHP on ED.[der_provider_code] = vOPHP.organisation_code 
--Look up Site
left join [UKHD_ODS].[Provider_site]  as vOPHS on ED.[Site_Code_of_Treatment] = vOPHS.Site_code
--Look up Purchaser
left join [MESH_ECDS].[EC_2425_Der] der on der.EC_Ident = ed.EC_ident and der.[Der_Financial_Year] = ed.[Der_Financial_year]
left join [UKHD_ODS].[Commissioner_Hierarchies_ICB] as rescom on rescom.[Organisation_Code] = der.[Responsible_Purchaser_Code] 
--Look up PAtient ICB
left join [MESH_ECDS].[EC_CCG] ccg on  ccg.EC_Ident = ed.EC_ident and ccg.[Der_Financial_Year] = ed.[Der_Financial_year]
--look up GP
left join [UKHD_ODS].[GP_Hierarchies_All] as vOGP on ed.GP_Practice_Code = vOGP.GP_Code and [Rel_Active] = 'TRUE' --get latest record with active PCN
--Get more date info
